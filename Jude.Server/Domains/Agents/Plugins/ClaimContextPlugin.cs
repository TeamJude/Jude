...
public class ClaimContextPlugin
{
    ...
    public async Task<string> GetActiveRulesAsync()
    {
        try
        {
            var activeRules = await _dbContext.Rules
                .Where(r => r.Status == Data.Models.RuleStatus.Active)
            ...
            _logger.LogDebug("Retrieved {RuleCount} active rules for agent context", activeRules.Count);
            return rulesText;
        }
        catch (Exception ex)
        {
    ...
    [KernelFunction]
    [Description("Get all active fraud indicators and detection criteria to help identify potentially fraudulent claims")]
    public async Task<string> GetFraudIndicatorsAsync()
    {
        try
        {
            var fraudIndicators = await _dbContext.FraudIndicators
                .Where(f => f.Status == Data.Models.IndicatorStatus.Active)
                .Select(f => new
                {
                    f.Name,
                    f.Description
            ...
            _logger.LogDebug("Retrieved {IndicatorCount} fraud indicators for agent context", fraudIndicators.Count);
    ...
    public async Task<string> GetPolicyGuidelinesAsync()
    {
    ...
    [KernelFunction]
    [Description("Get information about common billing codes and their typical costs for comparison")]
