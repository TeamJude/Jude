	...
	useEffect(() => {
		if (claim.status === "inProgress" && isStreaming) {
			const interval = setInterval(() => {
				...
				if (Math.random() < 0.3) {
					// 30% chance to add new reasoning
					const randomReasoning =
						newReasonings[Math.floor(Math.random() * newReasonings.length)];
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
							<Card className="p-4">
								...
								<div className="space-y-1 text-sm">
									<p>
										<strong>Name:</strong> {claim.patientName}
									</p>
									<p>
										<strong>Member ID:</strong> {claim.membershipNumber}
									</p>
									{claim.patientDetails && (
										<>
											<p>
												<strong>DOB:</strong> {claim.patientDetails.dateOfBirth}
											</p>
											<p>
												<strong>Gender:</strong> {claim.patientDetails.gender}
											</p>
											<p>
												<strong>ID Number:</strong>{" "}
												{claim.patientDetails.idNumber}
											</p>
										</>
									)}
								</div>
							...
							<Card className="p-4">
								...
								<div className="space-y-1 text-sm">
									<p>
										<strong>Practice:</strong> {claim.providerPractice}
									</p>
									<p>
									...
									{claim.providerDetails && (
										<>
											<p>
												<strong>Name:</strong> {claim.providerDetails.name}
											</p>
											<p>
												<strong>Contact:</strong>{" "}
												{claim.providerDetails.contactInfo}
											</p>
										</>
									)}
								</div>
					...
					{claim.items && claim.items.length > 0 && (
						<section>
							...
							<Accordion>
								<AccordionItem
									key="items"
									title={`${claim.items.length} items`}
								>
