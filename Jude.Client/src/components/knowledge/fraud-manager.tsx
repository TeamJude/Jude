import {
	createFraudIndicator,
	getFraudIndicators,
} from "@/lib/services/fraud.service";
...
export const FraudManager: React.FC = () => {
	...
	const [editingIndicator, setEditingIndicator] =
		useState<FraudIndicator | null>(null);
			...
			<Card className="shadow-sm border-zinc-200 border-1">
				<CardBody>
					...
					>
						...
						>
							{data?.success
								? data.data.fraudIndicators.map((indicator) => (
					...
					{(onClose) => (
						<>
							<ModalHeader className="flex flex-col gap-1">
								{editingIndicator
									? "Edit Fraud Indicator"
									: "Create New Fraud Indicator"}
							</ModalHeader>
							<form
								onSubmit={(e) => {
									e.preventDefault();
											...
											<form.Field name="status">
												{(field) => (
													<div className="flex items-center h-[40px]">
														<Switch
															isSelected={
																field.state.value === IndicatorStatus.Active
															}
