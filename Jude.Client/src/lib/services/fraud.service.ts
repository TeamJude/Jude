...
const getFraudIndicators = async (data: {
    page: number;
    pageSize: number;
}): Promise<ApiResponse<{ fraudIndicators: FraudIndicator[]; totalCount: number }>> => {
    return apiRequest<{ fraudIndicators: FraudIndicator[]; totalCount: number }>(`/api/fraud?page=${data.page}&pageSize=${data.pageSize}`, {
        method: "GET",
    });
};
